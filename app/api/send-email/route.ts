...
import { getCourseUrlsById } from "@/utils";
...
export async function POST(req: Request) {
  ...
  try {
    const { name, email, courseTitle, courseId } = await req.json();

    if (!name || !email || !courseTitle || !courseId) {
          ...
          .map(
            (item) => `
              <li style="margin-bottom: 10px;">
                <strong style="color: #4f46e5;">${item?.title}</strong>:<br/>
                <a href="${item?.Url}" style="color: #1d4ed8; text-decoration: none;">${item?.Url}</a>
              </li>`
          )
          .join("")}
      </ul>
